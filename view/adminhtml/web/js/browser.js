define([
    ...
    window.MediabrowserUtility = {
        ...
        imgixWindowId: 'imgix_modal_dialog_message',
        modalLoaded: false,
        imgixModalLoaded: false,
         ...
         * @param {Object} options
         */
         openImgixDialog: function (url, width, height, title, options) {
            var imgixWindowId = this.imgixWindowId,
                content = '<div class="popup-window imgix-modal-dialog-message" id="' + imgixWindowId + '"></div>';

            this.lastOpenModal = "imgixModal";
            $("meta[name=viewport]").attr("content", "width=device-width, initial-scale=1.0");
            if (this.imgixModalLoaded) {

                if (!_.isUndefined(options)) {
                    this.imgixModal.modal('option', 'closed', options.closed);
                }

                this.imgixModal.modal('openModal');
                this.setTargetElementId(options, url);
                this.setPathId(url);
                $(window).trigger('reload.MediaGallery');
                this.imgixModal.trigger('contentUpdated');

                return;
            }
            
            this.imgixModal = $(content).modal($.extend({
                title:  title || 'Add imgix image...',
                modalClass: 'magento mage-new-imgix-dialog',
                type: 'slide',
                buttons: []
            }, options));
            ...
            $.ajax({
                ...
                this.imgixModal.html(data).trigger('contentUpdated');
                this.imgixModalLoaded = true;
                this.setTargetElementId(options, url);
                this.setPathId(url);
            }.bind(this));
        ...
        closeDialog: function () {
            if(this.lastOpenModal == "imgixModal"){
                this.imgixModal.modal('closeModal');
    ...
    $.widget('mage.mediabrowser', {
        ...
        loadFileList: function (node, uploaded) {
            ...
            if (node !==null ){
                return $.ajax({
                    ...
                    if (uploaded) {
                        contentBlock.find('.imgix-asset:last').click();
                    }
                });
            }
        },
        ...
        selectFile: function (event) {
            ...
            if ($('#insert_files').hasClass("btn-disable")){
                $('#insert_files').attr("disabled","disabled");
            } else {
                $('#insert_files').removeAttr("disabled");
            }
        ...
        insert: function (event) {
            ...
            var attr_data_url = active_node = imageurl = store_id = null ;
            
            attr_data_url = fileRow.attr('data-url');
            
            if (typeof attr_data_url !== 'undefined' && attr_data_url !== false) {
                active_node = null;
                imageurl = attr_data_url;
            } else {
                active_node = this.activeNode.id;
            }
